...
public class KafkaUserOperatorMockTest {
    ...
    public void testCreateTlsUser() throws ExecutionException, InterruptedException {
        ...
        assertThat(new String(Base64.getDecoder().decode(userSecret.getData().get("user.crt"))), is("crt file"));
        assertThat(new String(Base64.getDecoder().decode(userSecret.getData().get("user.key"))), is("key file"));
    ...
    public void testCreateTlsUserWithACLsDisabled() throws ExecutionException, InterruptedException {
        ...
        assertThat(userSecret.getMetadata().getLabels(),
                is(Labels.fromMap(user.getMetadata().getLabels())
                        ...
                        .toMap()));
        assertThat(new String(Base64.getDecoder().decode(userSecret.getData().get("ca.crt"))), is("clients-ca-crt"));
        assertThat(userSecret.getData().get("user.crt"), is(MockCertManager.serverCert()));
        assertThat(userSecret.getData().get("user.key"), is(MockCertManager.serverKey()));
    ...
    public void testCreateTlsUserWithSecretPrefix() throws ExecutionException, InterruptedException {
        ...
        assertThat(prefixedUserSecret.getData().get("user.crt"), is(MockCertManager.serverCert()));
        assertThat(prefixedUserSecret.getData().get("user.key"), is(MockCertManager.serverKey()));
